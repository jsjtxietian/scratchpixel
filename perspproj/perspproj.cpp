//This program renders a wireframe image of a 3D object whose description
//is stored in the program itself. The result is stored to a SVG file.
//To draw an image of each triangle making up that object, we project
//the vertices of each triangle onto the screen using perspective
//projection (effectively transforming the vertices world coordinates
//to 2D pixel coordinates). Triangles are stored in the SVG file by
//connecting their respective vertices to each other with lines.
...
// Triangle index array. A triangle has 3 vertices. Each successive group of 3 integers in this array represent the positions of the vertices in the vertex array making up one triangle of that object. For example, the first 3 integers from this array, 8/7/9 represent the positions of the vertices making up the the first triangle. You can access these vertices with the following code:
//     verts[8]; /* first vertex  */
//     verts[7]; /* second vertex */
//     verts[9]; /* third vertex  */
// 6/5/5 are the positions of the vertices in the vertex array making up the second triangle, and so on. To find the indices of the n-th triangle, use the following code:
...
//Compute the 2D pixel coordinates of a point defined in world space.
//This function requires the point original world coordinates of course,
//the world-to-camera matrix (which you can get from computing the inverse
//of the camera-to-world matrix, the matrix transforming the camera),
//the canvas dimension and the image width and height in pixels.
// Note that we don't check in this version of the function if the point
//is visible or not. If the absolute value of of any of the screen
//coordinates are greater that their respective maximum value (the canvas
//width for the x-coordinate, and the canvas height for the y-coordinate)
//then the point is not visible. When a SVG file is displayed to the screen,
// the application displaying the content of the file clips points and lines
// which are not visible. Thus we can store lines or point in the file
// whether they are visible or not. The final clipping will be done when
//the image is displayed to the screen.

void computePixelCoordinates(const Vec3f pworld, Vec2i &pRaster, const Matrix44f &worldToCamera,
...
int main()
{
    ...
    //We exported the camera matrix from Maya. We need to compute its inverse, which is the matrix used in the computePixelCoordinates() function.
