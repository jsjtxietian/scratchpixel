...
{
  public:
	Object() : materialType(DIFFUSE_AND_GLOSSY),
	...
	bool test = false)
{
	...
	if (trace(orig, dir, objects, tnear, index, uv, &hitObject))
	{
		...
		case REFLECTION_AND_REFRACTION:
		{
			...
			Vec3f reflectionRayOrig = (dotProduct(reflectionDirection, N) < 0) ? hitPoint - N * options.bias : hitPoint + N * options.bias;
			Vec3f refractionRayOrig = (dotProduct(refractionDirection, N) < 0) ? hitPoint - N * options.bias : hitPoint + N * options.bias;
...
int main(int argc, char **argv)
{
	...
	std::cerr << "Primary : " << primaryNum << '\n'
			  << "Reflection : " << reflectionNum << '\n'
			  << "Refraction : " << refractionNum << '\n'
			  << "Shadow : " << shadowNum << std::endl;
