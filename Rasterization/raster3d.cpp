...
//space. They are then converted to camera space, then to NDC space
//(in the range [-1,1]) and then to raster space. The z-coordinates of
//the vertex in raster space is set with the z-coordinate of the vertex
//in camera space.
...
int main()
{
    ...
    for(uint32_t i = 0; i < ntris; ++i)
    {
        ...
        //Prepare vertex attributes. Divde them by their vertex z-coordinate (though we use
        //a multiplication here because v.z = 1 / v.z)
            ...
            for (uint32_t x = x0; x <= x1; x++)
            {
                ...
                if (w0 >= 0 && w1 >= 0 && w2 >= 0)
                {
                    ...
                    if (z < depthBuffer[y * imageWidth + x])
                    {
                        ...
                        //Proceed like with the other vertex attribute. Divide the point coordinates by the
                        //vertex z-coordinate then interpolate using barycentric coordinates and finally
                        //multiply by sample depth.
