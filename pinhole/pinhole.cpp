//This program implements a physical
//pinhole camera model similar to the model used in popular 3D packages such as Maya.

//Instructions to compile this program:
//c++ pinhole.cpp -o pinhole -std=c++11
//Run with: ./pinhole. Open the file ./pinhole.svg in any Internet browser to see the result.
...
//List of certices making up the object
...
// Compute the 2D pixel coordinates of a point defined in world space. This function requires
//the point original world coordinates of course, the world-to-camera matrix (which you can get
//from computing the inverse of the camera-to-world matrix, the matrix transforming the camera),
//the canvas dimension and the image width and height in pixels.The canvas coordinates (bottom,
//left, top, right) are also passed to the function so we can test the point raster coordinates
//against the canvas coordinates. If the point in raster space lies within the canvas boundaries,
...
int main()
{
    ...
    //at the near clipping plane. By changing the near clipping plane value, you will change
    //the position of the canvas, but this won't change the output of the program.
    ...
    // Project the triangles vertices onto the plane. If at least one of the vertices lies outside the canvas boundaries (if the function computePixelCoordinates returns false), the triangle is drawn in red and black otherwise. The result is store in a SVG file.
    std::ofstream ofs;
