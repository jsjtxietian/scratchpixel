...
//Returns true if the ray intersects an object. The variable tNear is set to the closest intersection distance and hitObject is a pointer to the intersected object.
//The variable tNear is set to infinity and hitObject is set null if no intersection was found.
    ...
    const std::vector<std::unique_ptr<Object>> &objects)
{
    ...
    //Don't forget to transform the ray origin (which is also the camera origin by transforming the point with coordinates (0,0,0) to world-space using the camera-to-world matrix.
    Vec3f orig;
    ...
    for (uint32_t j = 0; j < options.height; ++j)
    {
        ...
        {
        //Generate primary ray direction. Compute the x and y position of the ray in screen space.
        //This gives a point on the image plane at z=1. From there, we simply compute the direction
        //by normalized the resulting vec3f variable. This is similar to taking the vector between
        //the point on the image plane and the camera origin, which in camera space is (0,0,0):
...
//In the main function of the program, we create the scene (create objects) as well as set the options
//for the render (image widht and height etc.). We then call the render function().
