...
//static const float M_PI_2 = 1.57079632679f;
...
public:
		...
		std::unique_ptr<Vec2f[]>& st) : numTris(0)
	{
		...
		{
			P[i] = verts[i];
			bounds[0].x = bounds[0].x < verts[i].x ? bounds[0].x : verts[i].x;
			bounds[0].y = bounds[0].y < verts[i].y ? bounds[0].y : verts[i].y;
			bounds[0].z = bounds[0].z < verts[i].z ? bounds[0].z : verts[i].z;
			bounds[1].x = bounds[1].x > verts[i].x ? bounds[1].x : verts[i].x;
			bounds[1].y = bounds[1].y > verts[i].y ? bounds[1].y : verts[i].y;
			bounds[1].z = bounds[1].z > verts[i].z ? bounds[1].z : verts[i].z;
		}

		// allocate memory to store triangle indices
		trisIndex = std::unique_ptr<uint32_t[]>(new uint32_t[numTris * 3]);
		uint32_t l = 0;
		// Generate the triangle index array Keep in mind that there is
		//generally 1 vertex attribute for each vertex of each face.
		// So for example if you have 2 quads, you only have 6 vertices but
		//you have 2 * 4 vertex attributes (that is 8 normals, 8 texture
		//coordinates, etc.). So the easiest lazziest method in our triangle mesh,
		//is to create a new array for each supported vertex attribute (st, normals, etc.)
		//whose size is equal to the number of triangles multiplied by 3, and then
		//set the value of the vertex attribute at each vertex of each triangle using
		//the input array (normals[], st[], etc.)
...
//The main render function. This where we iterate over all pixels in the image, generate primary
//rays and cast these rays into the scene. The content of the framebuffer is saved to a file.
...
// set the options for the render (image widht and height, maximum recursion depth, field-of-view, etc.).
...
int main(int argc, char** argv)
{
	...
	Matrix44f tmp = Matrix44f(0.707107, -0.331295, 0.624695, 0, 0, 0.883452, 0.468521, 0, -0.707107, -0.331295, 0.624695,
	                          0, -1.63871, -5.747777, -40.400412, 1);
